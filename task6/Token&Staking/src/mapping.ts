...
export function handleApproval(event: ApprovalEvent): void {
  ...
  let user = UserToken.load(entity._owner.toHexString());
...
export function handleTransfer(event: TransferEvent): void {
  ...
  let sender = UserToken.load(entity._from.toHexString());
  let receiver = UserToken.load(entity._to.toHexString());
  ...
  if (!sender) {
    sender = new UserToken(entity._from.toHexString());
  ...
  if (!receiver) {
    receiver = new UserToken(entity._to.toHexString());
    tokenDetail._holders = tokenDetail._holders.plus(BigInt.fromI32(1));
  } else if (
